...
void
vminit(pagetable_t pgtbl) // 初始化指定的内核页表
{
  //   这里原来是只对全局内核页表进行初始化
  ...
  // CLINT
  // 查阅 xv6 book 的 Chapter 5 以及 start.c 可以知道 CLINT 仅在内核启动的时候
  // 需要使用到，而用户进程在内核态中的操作并不需要使用到该映射
...
// 这里参考下面的uvmcopy函数，但不拷贝实际的物理内存
// 将 src 页表的一部分页映射关系拷贝到 dst 页表中
// 只拷贝页表项，不拷贝实际的物理页内存
// 从 start 开始，拷贝 sz 大小的部分到dst中
int 
kvmcopymappings(pagetable_t src, pagetable_t dst, uint64 start, uint64 sz) 
{
  ...
  for (i = PGROUNDUP(start); i < start + sz; i += PGSIZE) {
    if ((pte = walk(src, i, 0)) == 0)   // 获取 i 对应的页表项，若不存在则panic
      panic("kvmcopymappings: pte should exist");
    if ((*pte & PTE_V) == 0)    // 检查页表项是否有效，无效(未加载到内存)则panic
...
kvmdealloc(pagetable_t pagetable, uint64 oldsz, uint64 newsz) {
  ...
  if(PGROUNDUP(newsz) < PGROUNDUP(oldsz)){
    ...
    // 与uvmdealloc的区别在于uvmunmap的最后一个标志设为0
    // 设置为0表示不清除实际的物理内存
